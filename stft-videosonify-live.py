...
jf = jackfiller.JackFiller(inputs=False,outputs=True,channels=1)
jf.activate()
jf.connect_to_physical_ports()
sr = 44100
wt = osr/float(window_size)
srones = np.ones(int(float(sr)/wt))
def resize_audio(audio):
    return np.kron(audio, srones)
...
while(running):
    ...
    if (dooverlaps):
        ...
        aud = audio[0:olaps]/amax
    else:
        aud = audio/amax
    outwav.write_frames(resize_audio(aud))
    #outwav.write_frames(aud)
    jf.add(aud)
    #outwav.write_frames(windowed)
